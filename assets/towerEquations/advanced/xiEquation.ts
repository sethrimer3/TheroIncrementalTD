...
export const xi = {
  ...
  variables: [
    {
      ...
      getSubEquations({ blueprint, towerId, value }) {
        ...
        const chains = 3 + Math.max(0, isFiniteNumber(chainsRaw) ? Math.round(chainsRaw) : 0);
        const exponentRaw = helpers.computeTowerVariableValue?.(towerId, 'aleph5', effectiveBlueprint);
        const exponent = 1 + 0.1 * Math.max(0, isFiniteNumber(exponentRaw) ? exponentRaw : 0);
  ...
  computeResult(values) {
    ...
    const aleph5 = Math.max(0, isFiniteNumber(values.aleph5) ? values.aleph5 : 0);
