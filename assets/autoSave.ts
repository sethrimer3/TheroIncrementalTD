...
// Storage key used to persist the spendable Aleph Glyph (Tower glyph) balance.
...
// Compatibility key used to persist Tower of Inspiration mote glow settings.
...
// Storage key used to persist the crystal background sprites toggle preference.
...
/**
 * Preference snapshot shape returned by `getPreferenceSnapshot`. Every field is
 * optional/nullable because callers only persist the values a given snapshot
 * actually populated (preserving the original `undefined`/`null` skip checks).
 */
export interface AutoSavePreferenceSnapshot {
  notation?: string | null;
  graphics?: string | null;
  glyphEquations?: string | null;
  damageNumbers?: string | null;
  waveKillTallies?: string | null;
  waveDamageTallies?: string | null;
  trackTracer?: string | null;
}

/**
 * Options accepted by `applyNotationPreference`/preference-apply style hooks:
 * `persist: false` is used when restoring from storage to avoid re-writing the
 * same value that was just read.
 */
export interface AutoSaveApplyOptions {
  persist?: boolean;
}

/**
 * Opaque, unmodeled snapshot payload for spire/game-state subsystems whose
 * schemas are owned elsewhere. Kept as a narrow typed adapter rather than a
 * fully modeled interface (see file header note and Phase 2 Decision Log entry).
 */
export type AutoSaveSnapshot = Record<string, unknown>;

/**
 * Injected runtime hooks the autosave helpers call into so they can read and
 * write broader game state without introducing circular imports. All entries
 * are optional because `configureAutoSave` is called incrementally by
 * different subsystems, each wiring only the hooks relevant to it.
 */
export interface AutoSaveDependencies {
  getPowderCurrency: () => number;
  onPowderCurrencyLoaded: (value: number) => void;
  applyStoredAudioSettings: ((settings: AutoSaveSnapshot) => void) | null;
  syncAudioControlsFromManager: (() => void) | null;
  applyNotationPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  handleNotationFallback: (() => void) | null;
  applyGlyphEquationPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyDamageNumberPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyDamageNumberMode: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyWaveKillTallyPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyWaveDamageTallyPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyTrackTracerPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyFrameRateLimitPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  applyFpsCounterPreference: ((value: string, options?: AutoSaveApplyOptions) => void) | null;
  getGameStatsSnapshot: (() => AutoSaveSnapshot) | null;
  mergeLoadedGameStats: ((stats: AutoSaveSnapshot) => void) | null;
  getPreferenceSnapshot: (() => AutoSavePreferenceSnapshot) | null;
...
/**
 * Configuration payload accepted by `configureAutoSave`. `statKeys` (or a
 * `gameStatsTemplate` object whose keys are used instead) determines which
 * game-stat fields are persisted/restored; any subset of `AutoSaveDependencies`
 * may also be supplied to wire additional hooks incrementally.
 */
export type AutoSaveConfig = Partial<AutoSaveDependencies> & {
  statKeys?: string[];
  gameStatsTemplate?: Record<string, unknown>;
};

const dependencies: AutoSaveDependencies = {
  getPowderCurrency: () => 0,
...
let statKeys: string[] = [];
let powderSaveHandle: ReturnType<typeof setTimeout> | null = null;
let powderBasinSaveHandle: ReturnType<typeof setTimeout> | null = null;
 ...
 */
export function configureAutoSave(config: AutoSaveConfig = {}): void {
 ...
 * entry is valid.
 */
export function readStorageJson<T = unknown>(key: string): T | null {
 ...
 */
export function savePowderCurrency(): void {
  if (powderSaveHandle) {
 ...
 */
export function schedulePowderSave(): void {
  if (powderSaveHandle) {
    return;
  }
...
export function loadPersistentState(): void {
  const storedPowder = readStorageJson<number>(POWDER_STORAGE_KEY);
  if (Number.isFinite(storedPowder) && typeof dependencies.onPowderCurrencyLoaded === 'function') {
    dependencies.onPowderCurrencyLoaded(Math.max(0, storedPowder as number));
  }

  if (typeof dependencies.applyPowderBasinSnapshot === 'function') {
    const storedBasin = readStorageJson<AutoSaveSnapshot>(POWDER_BASIN_STORAGE_KEY);
  ...
  if (dependencies.audioStorageKey && typeof dependencies.applyStoredAudioSettings === 'function') {
    const storedAudio = readStorageJson<AutoSaveSnapshot>(dependencies.audioStorageKey);
    if (storedAudio) {
  ...
  if (statKeys.length && typeof dependencies.mergeLoadedGameStats === 'function') {
    const storedStats = readStorageJson<Record<string, unknown>>(GAME_STATS_STORAGE_KEY);
...
function persistGameStats(): void {
...
function persistTowerUpgrades(): void {
...
function persistLevelProgress(): void {
 ...
 * predictable cadence.
 */
export function startAutoSaveLoop({ intervalMs = DEFAULT_AUTOSAVE_INTERVAL_MS }: { intervalMs?: number } = {}): void {
 ...
 */
export function commitAutoSave(): void {
  performAutoSave();
}

// Persist the current Spire layout alongside player stats.
function persistPowderBasin(): void {
 ...
 */
export function schedulePowderBasinSave(): void {
  if (powderBasinSaveHandle) {
    return;
  }
