...
/** Match Number.isFinite's number-only acceptance while narrowing optional context output. */
function isFiniteNumber(value) {
    return typeof value === 'number' && Number.isFinite(value);
}
/** Spiral-hit blueprint for the Tau tower. */
export const tau = {
    ...
    variables: [
        {
            ...
            getSubEquations() {
                const helpers = ctx();
                const gammaValue = Math.max(0, Number.isFinite(helpers?.calculateTowerEquationResult?.('gamma'))
                    ? Number(helpers?.calculateTowerEquationResult?.('gamma'))
                    : 0);
                return [
            ...
            computeValue() {
                ...
                return Math.max(0, isFiniteNumber(gammaValue) ? gammaValue : 0);
            },
        },
        {
            key: 'aleph1',
    ...
    computeResult(values) {
        ...
        const particles = Math.max(1, 1 + Math.floor(Number(values.aleph3 || 0)));
