...
/** Invoke the required presenter upgrade-state helper with its native failure boundary. */
function ensureUpgradeState(towerId, blueprint) {
    const ensureState = ctx().ensureTowerUpgradeState;
    if (typeof ensureState !== 'function') {
        throw new TypeError('ctx(...).ensureTowerUpgradeState is not a function');
    }
    return ensureState(towerId, blueprint);
}
export const lambda = {
    ...
    variables: [
        {
            ...
            getSubEquations({ blueprint, towerId, value }) {
                ...
                const enemyWeight = isFiniteNumber(enemyWeightRaw) ? Math.max(1, enemyWeightRaw) : 1;
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const state = ensureUpgradeState(towerId, blueprint);
    ...
    computeResult(values) {
        const kappa = isFiniteNumber(values.kappa) ? values.kappa : 0;
        const enemyWeight = isFiniteNumber(values.enemyWeight) ? values.enemyWeight : 1;
