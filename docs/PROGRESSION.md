...
| Ζ / ζ | `ζ = Atk × Crt × Spd` drives two large tracers: `r(θ)=Rs|sin(kθ+φ)|` and a configurable Lissajous-style `(x(t), y(t))`. | Zeta lets players freely change petals, phases, frequencies, and sine/cosine coefficients like a graphing calculator; only Attack, Critical Multiplier, and Speed remain upgrade-driven. | **Ζ** projects both a polar curve and dotted parametric component guides; projectile heads crit while trails deal base attack damage. The original pendulum is preserved separately as **Zeta (OLD)**. |
...
| Π / π | `π = ο^{|θ|/(100-Bet₁)}` locks beams onto enemies and scales damage with rotation. | Pi fires `numLaser = 2 + Lamed₁` beams that track enemies; damage grows exponentially as each beam rotates around the tower. | **Π** doubles the rotation scaling exponent and extends lock-on persistence for 2s after enemies leave range. |
...
| Σ / σ | `σ = ρ + ln(ρ + 1)` now manifests as a friendly-fire well where `Atk = storedDmg`. | Sigma attracts nearby allies to shoot it, storing their damage before unleashing the entire sum as a single shot. | **Σ** keeps the accumulator intact after each shot, continuously releasing stored damage without resetting the pool. |
...
| Υ / υ | `υ = atk × tot × spd` dispatches an infinite-range micro-triangle fleet. | Upsilon launches curved fighter swarms that lock onto focused enemies and coast back to the tower when lanes are clear. | **Υ** preserves the global focus and redeploys the hangar in a brighter burst after every recall. |
