...
The active game has one surviving Spire experience: the **Tower of Inspiration**, formerly presented as the Aleph Spire. Achievements and the Achievements tab remain active.
...
The user-facing name is **Tower of Inspiration**. Compatibility-sensitive internal names remain in place where renaming would create a broad migration: `wellOfInspiration`, `powder`, `aleph`, `alephSpire`, `powderState`, `PowderSimulation`, Aleph wall/tier fields, Aleph glyph IDs, the `powder` tab and panel IDs, and existing `glyph-defense-idle:*` storage keys. `assets/saveCompatibility.ts` and `assets/state/spireResourceState.ts` normalize historical save shapes while ignoring retired branches.
